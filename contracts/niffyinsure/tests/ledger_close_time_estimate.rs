...
fn admin_can_update_estimate() {
    let (_env, client, _admin) = setup();
    client.admin_set_ledger_close_est(&7u32).unwrap();
...
fn estimate_zero_is_rejected() {
    let (_env, client, _admin) = setup();
    assert!(client.try_admin_set_ledger_close_est(&0u32).is_err());
}

#[test]
fn estimate_above_thirty_is_rejected() {
    let (_env, client, _admin) = setup();
    assert!(client.try_admin_set_ledger_close_est(&31u32).is_err());
}

#[test]
fn estimate_at_boundary_is_accepted() {
    let (_env, client, _admin) = setup();
    client.admin_set_ledger_close_est(&1u32).unwrap();
    assert_eq!(client.get_ledger_close_time_estimate(), 1u32);
    client.admin_set_ledger_close_est(&30u32).unwrap();
